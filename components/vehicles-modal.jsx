      ...
      <div className="relative top-24 bottom-12 mx-auto px-5 lg:px-10 py-5 w-4/5 lg:w-1/2 shadow-lg rounded-md bg-white dark:bg-midnight-blue">
        ...
        <div className="text-center">
          <h2 className="font-bold dark:font-medium text-xl text-slate-800 dark:text-white">
            {" "}
            {searching ? "Tracking Vehicle" : "Location Found"}{" "}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {" "}
            {searching
              ? "Please hold on. Locating vehicle on AMINCI Network."
              : "Vehicle has been successfully located."}
          </p>
        </div>
            ...
            <div className={searching ? "mx-0" : "mx-auto md:mx-0"}>
              <div className="flex py-3 w-56">
                <div className="mr-4 w-12 h-12 rounded-full bg-slate-800">
                  <Image
                    ...
                    src={toggleModal ? trackedVehicle.img : "/user.jpg"}
                    alt={trackedVehicle.transporter}
                  />
                </div>
                <span>
                  <span className="font-bold dark:font-normal text-sm block dark:text-white">
                    {" "}
                    {trackedVehicle.transporter}{" "}
                  </span>
                  <span className="text-xs text-slate-500"> Transporter </span>
                </span>
                {!searching && (
                  <button>
                    <span className="ml-5 text-xs border border-slate-600 dark:border-slate-400 dark:text-slate-400 py-1 px-5 rounded-full">
                      View
                    </span>
                  </button>
                )}
              ...
              {!searching && (
                <div className="px-3 py-3 border-y border-slate-300 dark:border-slate-500">
                  <span className="font-bold dark:font-normal dark:text-white text-sm block">
                    {" "}
                    {trackedVehicle.currentLocation}, NG{" "}
                  </span>
            ...
            {!searching && (
              <div className="mx-auto w-80 h-48 bg-slate-400 dark:bg-evening-blue overflow-hidden">
                <iframe
                  width={320}
                  height={192}
                  src={`https://www.google.com/maps/embed/v1/place?key=AIzaSyCfDCwk8E9_xavNnEWV8Zg7nGcxvY0xh0A&q=${trackedVehicle.currentLocation}`}
                  alt="map"
                  loading="lazy"
                ></iframe>
              </div>
          ...
          <div className="flex px-2 my-2">
            <div className="flex">
              ...
              <span>
                <span className="font-bold dark:font-normal dark:text-white text-sm block">
                  {" "}
                  {trackedVehicle.vehicleNumber}{" "}
                </span>
                <span className="text-xs text-slate-500">
                  {" "}
                  {trackedVehicle.vehicleName}{" "}
                </span>
              </span>
            </div>
          </div>
